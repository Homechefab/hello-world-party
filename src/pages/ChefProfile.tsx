...
import { checkAvailability } from "@/hooks/useChefAvailability";
...
): { canOrder: boolean; message: string | null } => {
  const chefAvailability = operatingHours.length > 0
    ? checkAvailability(operatingHours)
    : { isOpen: true, nextOpenInfo: null };

  if (!chefAvailability.isOpen) {
    return {
      canOrder: false,
      message: chefAvailability.nextOpenInfo
        ? `Tar inte emot beställningar nu. Öppnar igen ${chefAvailability.nextOpenInfo}.`
        : 'Tar inte emot beställningar just nu.',
    };
  }

  const stockholmNow = new Date(new Date().toLocaleString('en-US', { timeZone: 'Europe/Stockholm' }));
  const currentDay = stockholmNow.getDay();
  const dishSchedule = schedules[dishId];

  if (dishSchedule && dishSchedule.length > 0) {
    const todaySchedule = dishSchedule.find((schedule) => schedule.day_of_week === currentDay);
    if (!todaySchedule?.is_available) {
      return {
        canOrder: false,
        message: 'Den här rätten går inte att beställa idag.',
      };
    }
  }
...
const ChefProfile = () => {
  ...
  const handleAddToCart = (dish: Dish) => {
    ...
    if (!orderStatus.canOrder) {
      toast({
        title: 'Kan inte beställas nu',
        description: orderStatus.message || 'Beställning är inte tillgänglig just nu.',
        variant: 'destructive',
      });
      return;
    }
            ...
            <div className="max-w-4xl mx-auto text-center relative">
              ...
              {chef.business_name === 'La Flamme' && (
                <div className="mt-4 max-w-2xl mx-auto text-center text-white/90 space-y-2">
                  <p>Beställ onsdag–fredag 17:00–22:00, minst 24 timmar i förväg.</p>
                </div>
              )}
